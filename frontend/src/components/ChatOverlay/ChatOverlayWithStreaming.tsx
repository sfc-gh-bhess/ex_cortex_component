...
import { ChatMessage as ChatMessageType, ChatOverlayProps, ResponseEvent, defaultDisplayConfig } from './types';
...
export const ChatOverlayWithStreaming: React.FC<ChatOverlayProps> = (props) => {
  ...
  // Initialize thread_id on first render
  useEffect(() => {
    if (!threadIdRef.current) {
      // Generate a random numeric thread ID using timestamp and random number
      threadIdRef.current = Date.now() + Math.floor(Math.random() * 1000000);
    }
  }, []);
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      // Stream responses
      for await (const { event, data } of streamAgentResponse(agentRequest)) {
        ...
        const isComplete = event === 'response' || event === 'done' || event === 'message_stop';
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantMessageId
              ? {
                  ...msg,
                  events: [...(msg.events || []), responseEvent],
                  isComplete,
                }
              : msg
          )
        ...
        // If this is the final response event, mark as complete
        if (isComplete) {
          setIsStreaming(false);
          currentStreamingMessageId.current = null;
          // Increment parent_message_id for next message
          setParentMessageId((prev) => prev + 1);
        }
      }
    } catch (error) {
      console.error('Streaming error:', error);
      
      // Add error event to the message
      ...
      setIsStreaming(false);
      currentStreamingMessageId.current = null;
      // Increment parent_message_id even on error for next message
      setParentMessageId((prev) => prev + 1);
    }
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Handle horizontal resizing (left side)
      if (direction.includes('left')) {
        newWidth = Math.max(300, startWidth - deltaX);
      }
      // Handle horizontal resizing (right side)
      if (direction.includes('right')) {
        newWidth = Math.max(300, startWidth + deltaX);
      }

      // Handle vertical resizing (top)
      if (direction.includes('top')) {
        newHeight = Math.max(400, startHeight - deltaY);
      }
      // Handle vertical resizing (bottom)
